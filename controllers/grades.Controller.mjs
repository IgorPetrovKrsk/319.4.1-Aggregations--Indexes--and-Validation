...
async function getStatsByClassId(req, res) {
    let collection = await db.collection("grades");

    let result = await collection
        .aggregate([
            {
               $match: { class_id: +req.params.id }, //filtering by class id
            },
            {
                $unwind: { path: "$scores" },
            },
...
export default { getAvgGrades, getGradesByClassId, getLearnerAvg, getStats,getStatsByClassId }